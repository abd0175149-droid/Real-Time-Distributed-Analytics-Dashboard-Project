...
-- MV 5-6: Page Metrics (CTE to avoid timestamp column name conflict)
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_page_1h TO page_metrics AS
WITH page_agg AS (
    SELECT toStartOfHour(timestamp) as ts, tracking_id, page_url,
    ...
    FROM page_events GROUP BY ts, tracking_id, page_url
)
SELECT ts as timestamp, '1h' as interval_type, tracking_id, page_url,
    pageviews, unique_visitors, avg_time_on_page_sec, avg_scroll_depth,
    total_clicks, avg_load_time_ms, p50_load_time_ms, p95_load_time_ms,
...
WITH page_agg AS (
    SELECT toStartOfDay(timestamp) as ts, tracking_id, page_url,
        count(*) as pageviews, uniq(session_id) as unique_visitors,
        avgIf(duration_ms, duration_ms IS NOT NULL) / 1000 as avg_time_on_page_sec,
        avgIf(scroll_depth_max, scroll_depth_max IS NOT NULL) as avg_scroll_depth,
        sum(coalesce(click_count, 0)) as total_clicks,
...
SELECT ts as timestamp, '1d' as interval_type, tracking_id, page_url,
    pageviews, unique_visitors, avg_time_on_page_sec, avg_scroll_depth,
    total_clicks, avg_load_time_ms, p50_load_time_ms, p95_load_time_ms,
    0 as entries, 0 as exits, 0 as bounces, now() as created_at
FROM page_agg;
...
FROM sessions GROUP BY toStartOfHour(start_time), tracking_id, device_type, operating_system, browser;
...
SELECT toStartOfDay(start_time) as timestamp, '1d' as interval_type, tracking_id,
    ...
    sum(page_views) as pageviews, avgIf(duration_ms, duration_ms IS NOT NULL) / 1000 as avg_session_duration_sec,
    if(count(*) > 0, countIf(bounce = 1) * 100.0 / count(*), 0) as bounce_rate, now() as created_at
FROM sessions GROUP BY toStartOfDay(start_time), tracking_id, device_type, operating_system, browser;
...
SELECT toStartOfHour(start_time) as timestamp, '1h' as interval_type, tracking_id,
    ...
    sum(page_views) as pageviews, avgIf(duration_ms, duration_ms IS NOT NULL) / 1000 as avg_session_duration_sec,
    if(count(*) > 0, countIf(bounce = 1) * 100.0 / count(*), 0) as bounce_rate, now() as created_at
FROM sessions GROUP BY toStartOfHour(start_time), tracking_id, country, country_code;
...
SELECT toStartOfDay(start_time) as timestamp, '1d' as interval_type, tracking_id,
    ...
    sum(page_views) as pageviews, avgIf(duration_ms, duration_ms IS NOT NULL) / 1000 as avg_session_duration_sec,
    if(count(*) > 0, countIf(bounce = 1) * 100.0 / count(*), 0) as bounce_rate, now() as created_at
FROM sessions GROUP BY toStartOfDay(start_time), tracking_id, country, country_code;
...
SELECT toStartOfHour(start_time) as timestamp, '1h' as interval_type, tracking_id,
    ...
    sum(page_views) as pageviews, avgIf(duration_ms, duration_ms IS NOT NULL) / 1000 as avg_session_duration_sec,
    if(count(*) > 0, countIf(bounce = 1) * 100.0 / count(*), 0) as bounce_rate, now() as created_at
FROM sessions GROUP BY toStartOfHour(start_time), tracking_id, referrer;
...
SELECT toStartOfDay(start_time) as timestamp, '1d' as interval_type, tracking_id,
    ...
    sum(page_views) as pageviews, avgIf(duration_ms, duration_ms IS NOT NULL) / 1000 as avg_session_duration_sec,
    if(count(*) > 0, countIf(bounce = 1) * 100.0 / count(*), 0) as bounce_rate, now() as created_at
FROM sessions GROUP BY toStartOfDay(start_time), tracking_id, referrer;

-- MV 13-14: Interaction Metrics (CTE to avoid timestamp conflict)
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_interaction TO interaction_metrics AS
WITH ia AS (
    SELECT toStartOfHour(timestamp) as ts, tracking_id, page_url, element, element_id, event_type,
        count(*) as total_interactions, uniq(user_id) as unique_users
    FROM interaction_events GROUP BY ts, tracking_id, page_url, element, element_id, event_type
)
SELECT ts as timestamp, '1h' as interval_type, tracking_id, page_url, element, element_id, event_type,
    total_interactions, unique_users, now() as created_at
FROM ia;

CREATE MATERIALIZED VIEW IF NOT EXISTS mv_interaction_1d TO interaction_metrics AS
WITH ia AS (
    SELECT toStartOfDay(timestamp) as ts, tracking_id, page_url, element, element_id, event_type,
        count(*) as total_interactions, uniq(user_id) as unique_users
    FROM interaction_events GROUP BY ts, tracking_id, page_url, element, element_id, event_type
)
SELECT ts as timestamp, '1d' as interval_type, tracking_id, page_url, element, element_id, event_type,
    total_interactions, unique_users, now() as created_at
FROM ia;

-- MV 15-16: Form Metrics (CTE to avoid timestamp conflict)
CREATE MATERIALIZED VIEW IF NOT EXISTS mv_form TO form_metrics AS
WITH fa AS (
    SELECT toStartOfHour(timestamp) as ts, tracking_id, form_id, coalesce(form_name, '') as form_name,
    ...
    FROM form_events GROUP BY ts, tracking_id, form_id, form_name
)
SELECT ts as timestamp, '1h' as interval_type, tracking_id, form_id, form_name,
    starts, submissions, successful_submissions, completion_rate, now() as created_at
FROM fa;

CREATE MATERIALIZED VIEW IF NOT EXISTS mv_form_1d TO form_metrics AS
WITH fa AS (
    SELECT toStartOfDay(timestamp) as ts, tracking_id, form_id, any(form_name) as form_name,
        countIf(event_type = 'form_focus') as starts,
        countIf(event_type = 'form_submit') as submissions,
    ...
    FROM form_events GROUP BY ts, tracking_id, form_id
)
SELECT ts as timestamp, '1d' as interval_type, tracking_id, form_id, form_name,
    starts, submissions, successful_submissions, completion_rate, now() as created_at
FROM fa;
...
WITH ecom_agg AS (
    SELECT 
        ...
        tracking_id,
        countIf(event_type = 'product_view') as pv,
        countIf(event_type = 'cart_add') as ca,
        countIf(event_type = 'cart_remove') as cr,
        countIf(event_type = 'checkout_step') as cs,
        countIf(event_type = 'purchase') as po,
        sumIf(total, event_type = 'purchase') as rev,
        uniqIf(user_id, event_type = 'purchase') as uc
    FROM ecommerce_events 
    GROUP BY ts, tracking_id
)
SELECT 
    ts as timestamp, 
    '1h' as interval_type, 
    tracking_id,
    CAST(pv AS UInt32) as product_views,
    CAST(ca AS UInt32) as cart_adds,
    CAST(cr AS UInt32) as cart_removes,
    CAST(cs AS UInt32) as checkouts,
    CAST(po AS UInt32) as total_orders,
    rev as total_revenue,
    CAST(if(po > 0, rev / po, 0) AS Float32) as avg_order_value,
    CAST(uc AS UInt32) as unique_customers,
    CAST(if(pv > 0, ca * 100.0 / pv, 0) AS Float32) as view_to_cart_rate,
    CAST(if(ca > 0, cs * 100.0 / ca, 0) AS Float32) as cart_to_checkout_rate,
    CAST(if(cs > 0, po * 100.0 / cs, 0) AS Float32) as checkout_to_purchase_rate,
    now() as created_at
FROM ecom_agg;
...
WITH ecom_daily AS (
    SELECT 
        toStartOfDay(timestamp) as day_ts,
        tracking_id,
        countIf(event_type = 'product_view') as pv,
        countIf(event_type = 'cart_add') as ca,
        countIf(event_type = 'cart_remove') as cr,
...
SELECT 
    ...
    CAST(cs AS UInt32) as checkouts,
    CAST(po AS UInt32) as total_orders,
    CAST(rev AS Float64) as total_revenue,
    CAST(if(po > 0, rev / po, 0) AS Float32) as avg_order_value,
    CAST(uc AS UInt32) as unique_customers,
    CAST(if(pv > 0, ca * 100.0 / pv, 0) AS Float32) as view_to_cart_rate,
    CAST(if(ca > 0, cs * 100.0 / ca, 0) AS Float32) as cart_to_checkout_rate,
...
GROUP BY toDate(timestamp), tracking_id, product_id;
...
SELECT toDate(timestamp) as date, tracking_id, video_src,
    ...
    now() as created_at
FROM video_events GROUP BY toDate(timestamp), tracking_id, video_src;
...
WITH page_sequence AS (
    ...
    GROUP BY toDate(timestamp), tracking_id, session_id
)
...
FROM page_events
WHERE event_type = 'page_view'
GROUP BY toDate(timestamp), tracking_id